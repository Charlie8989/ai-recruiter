import { pgSchema, pgTable, bigint, uuid, text, timestamp, varchar, boolean, json, jsonb, index, uniqueIndex, foreignKey, primaryKey, unique, check } from "drizzle-orm/pg-core"
...
export const feedback = pgTable("feedback", {
	...
	interviewid: varchar().references(() => interview.interviewid),
	feedback: json(),
...
export const interview = pgTable("interview", {
	...
	questionlist: json(),
	useremail: varchar(),
	interviewid: varchar().notNull(),
	name: text(),
}, (table) => [
	unique("interview_interviewid_key").on(table.interviewid),]);

export const lowercaseUsers = pgTable("users", {
	id: bigint({ mode: 'number' }).primaryKey().generatedAlwaysAsIdentity(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
	name: varchar(),
	email: varchar(),
	picture: varchar(),
	credits: bigint({ mode: 'number' }).default(3),
}, (table) => [
	unique("users_email_key").on(table.email),check("users_credits_check", sql`(credits >= 0)`),]);
